...
void CreateOutputFiles(int run_nmbr, int rank, ParamStruct* ps) {
	...
	sprintf(ps->state_file,"%s/states_%d_%d.out",ps->Chars[I_OUTPUT_STATE],rank,run_nmbr);
	sprintf(ps->grid_file,"%s/grid_%d",ps->Chars[I_OUTPUT_GRID],rank);

	//Delete existing states file
	/*int error_code = MPI_File_delete(ps->state_file,MPI_INFO_NULL);
	if (error_code != MPI_SUCCESS) {
		char error_string[255];
		int length_of_error_string;
		MPI_Error_string(error_code, error_string, &length_of_error_string);
		fprintf(stderr, "Error deleting existing states file: %3d: %s\n", rank, error_string);
	...
	if (pFile != NULL) {
		fprintf(pFile,"#Simulation variables:\nt_step t_final:");
		fprintf(pFile,"%f %f\n\n",ps->Reals[I_T_STEP],ps->Reals[I_T_FINAL]);
		fprintf(pFile,"#Global variables:\n");
		fclose(pFile);
	} else {
...
void WriteToGridFiles(int n, double t, GridStruct* gs, SimData* sd, ParamStruct* ps) {
	...
	if (ps->Integers[I_OUTPUT_GRID_FLAG] || (rank == 0 && run_nmbr == 0 && n == 0)) {
		char grid_file[255];
		sprintf(grid_file,"%s_%d_%d.vtk",ps->grid_file,run_nmbr,n);

		//Delete existing grid file
		//MPI_File_delete(grid_file,MPI_INFO_NULL);

		//pFile = fopen (grid_file,"w");
		int error_code = MPI_File_open(MPI_COMM_SELF,grid_file,MPI_MODE_WRONLY | MPI_MODE_CREATE, MPI_INFO_NULL, &fh);
...
*/
			...
			error_code = MPI_File_write(fh, buffer, ptr*sizeof(char), MPI_CHAR, &status);
			if (error_code != MPI_SUCCESS) {
				char error_string[255];
				int length_of_error_string;
				MPI_Error_string(error_code, error_string, &length_of_error_string);
				fprintf(stderr, "Error writing to grid file: %3d: %s\n", rank, error_string);
				MPI_Abort(MPI_COMM_WORLD,MPI_ERR_OTHER);
			}
			error_code = MPI_Barrier(MPI_COMM_SELF);
			if (error_code != MPI_SUCCESS) {
				char error_string[255];
				int length_of_error_string;
				MPI_Error_string(error_code, error_string, &length_of_error_string);
				fprintf(stderr, "Error with MPI_Barrier after grid write: %3d: %s\n", rank, error_string);
				MPI_Abort(MPI_COMM_WORLD,MPI_ERR_OTHER);
			}
			...
			int length_of_error_string;
			MPI_Error_string(error_code, error_string, &length_of_error_string);
			fprintf(stderr, "Error opening grid file < %s > : %3d: %s\n", grid_file, rank, error_string);
			MPI_Abort(MPI_COMM_WORLD,MPI_ERR_OTHER);
		}
	}
...
void WriteToStateFiles(double t, SimData* sd, ParamStruct* ps) {
	...
	//FILE * pFile;
	char* state_file = ps->state_file;
	//pFile = fopen (state_file,"a");
	int error_code = MPI_File_open(MPI_COMM_SELF,state_file,MPI_MODE_WRONLY | MPI_MODE_CREATE | MPI_MODE_APPEND, MPI_INFO_NULL, &fh);
...
*/
		...
		//MPI_File_write(fh, buffer, ptr*sizeof(char), MPI_CHAR, &status);
		error_code = MPI_File_write(fh, buf_real, ptr_real, MPI_FLOAT, &status);
		if (error_code != MPI_SUCCESS) {
			char error_string[255];
			int length_of_error_string;
			MPI_Error_string(error_code, error_string, &length_of_error_string);
			fprintf(stderr, "Error writing to states file: %3d: %s\n", rank, error_string);
			MPI_Abort(MPI_COMM_WORLD,MPI_ERR_OTHER);
		}
		error_code = MPI_Barrier(MPI_COMM_SELF);
		if (error_code != MPI_SUCCESS) {
			...
			int length_of_error_string;
			MPI_Error_string(error_code, error_string, &length_of_error_string);
			fprintf(stderr, "Error with MPI_Barrier after states write: %3d: %s\n", rank, error_string);
			MPI_Abort(MPI_COMM_WORLD,MPI_ERR_OTHER);
		}
		...
		error_code = MPI_File_close(&fh);
		if (error_code != MPI_SUCCESS) {
			char error_string[255];
			int length_of_error_string;
			MPI_Error_string(error_code, error_string, &length_of_error_string);
			fprintf(stderr, "Error closing states file: %3d: %s\n", rank, error_string);
			MPI_Abort(MPI_COMM_WORLD,MPI_ERR_OTHER);
		}
		//fclose(pFile);
	} else {
		...
		int length_of_error_string;
		MPI_Error_string(error_code, error_string, &length_of_error_string);
		fprintf(stderr, "Error opening states file: %3d: %s\n", rank, error_string);
		MPI_Abort(MPI_COMM_WORLD,MPI_ERR_OTHER);
	}
}
