...
*/
	delete[] Domain, V0, TTSurfaceArea, SRSurfaceArea, TropC, Ele_Nodes, Ele_1, Ele_2, RyR_Ele, RyR_JSR_Ele, LCC_Ele;
}
...
void InitializeDomainStruct(GridStruct* gs, int bSecondaryGradient, ParamStruct* ps) {
	...
	InitializeExplicitSparseMatrixFast(gs,0,0,0,0,ps);

	//fprintf(stdout,"Computing boundary rates...\n");
	//Boundary fluxes for diffusible states
	gs->Boundary_Rates = new double*[gs->N_States];
	for (int k = 0; k < gs->N_States; k++) {
		gs->Boundary_Rates[k] = new double[gs->N_Ele];
		for (int i = 0; i < gs->N_Ele; i++) {
			gs->Boundary_Rates[k][i] = 0;
		}
		for (int i = 0; i < gs->N_Face; i++) {
			int e = i;
			int tri1 = gs->Ele_1[e];
			if (gs->FaceType[e] == FACE_BOUND) {
				gs->Boundary_Rates[k][tri1] += ps->Diff[k] * (gs->Face_C[e]) * gs->Boundaries[k] / gs->V0[tri1];
			}
		}
	}

	//States buffer
	gs->States_Buffer = new double[gs->N_States*gs->N_Ele];
